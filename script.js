...
function switchPage(pageId) {
    ...
    if (activeNavItem) {
        const title = activeNavItem.querySelector('span').textContent;
...
function initToggleSwitches() {
    ...
    toggleSwitches.forEach(switchEl => {
        switchEl.addEventListener('click', () => {
            ...
            const controlItem = switchEl.closest('.control-item');
            const statusIndicator = controlItem.querySelector('.control-status');
            
            if (statusIndicator) {
                if (switchEl.classList.contains('active')) {
                    if (controlItem.querySelector('label').textContent.includes('Water Pump')) {
                        statusIndicator.textContent = 'Running';
                        statusIndicator.classList.add('active');
                    } else if (controlItem.querySelector('label').textContent.includes('Grow Lights')) {
...
function initSliders() {
    ...
    if (fanSpeedSlider && fanSpeedValue) {
        fanSpeedSlider.addEventListener('input', () => {
            ...
            // Update status indicator
            const controlItem = fanSpeedSlider.closest('.control-item');
            const statusIndicator = controlItem.querySelector('.control-status');
            if (statusIndicator) {
                statusIndicator.textContent = fanSpeedSlider.value + '%';
            }
        });
    }
    ...
    if (brightnessSlider && brightnessValue) {
        brightnessSlider.addEventListener('input', () => {
            ...
            // Update status indicator
            const controlItem = brightnessSlider.closest('.control-item');
            const statusIndicator = controlItem.querySelector('.control-status');
            if (statusIndicator) {
                statusIndicator.textContent = brightnessSlider.value + '%';
            }
        });
    }
}
        ...
        button.addEventListener('click', () => {
            ...
            switch(preset) {
                case 'morning':
                    ...
                    document.getElementById('brightnessValue').textContent = '60%';
                    document.querySelector('#brightness').closest('.control-item').querySelector('.control-status').textContent = '60%';
                    
                    document.getElementById('fanSpeed').value = 40;
                    document.getElementById('fanSpeedValue').textContent = '40%';
                    document.querySelector('#fanSpeed').closest('.control-item').querySelector('.control-status').textContent = '40%';
                    
                    // Turn on water pump if not already on
                    const waterPump = document.querySelector('.control-item label i.fa-shower').closest('.control-item').querySelector('.toggle-switch');
                ...
                case 'afternoon':
                    ...
                    document.querySelector('#brightness').closest('.control-item').querySelector('.control-status').textContent = '100%';
                    
                    document.getElementById('fanSpeed').value = 80;
                    document.getElementById('fanSpeedValue').textContent = '80%';
                    document.querySelector('#fanSpeed').closest('.control-item').querySelector('.control-status').textContent = '80%';
                    
                    // Turn off water pump if not already off
                    const waterPumpAfternoon = document.querySelector('.control-item label i.fa-shower').closest('.control-item').querySelector('.toggle-switch');
                ...
                case 'evening':
                    ...
                    document.querySelector('#brightness').closest('.control-item').querySelector('.control-status').textContent = '70%';
                    
                    document.getElementById('fanSpeed').value = 60;
                    document.getElementById('fanSpeedValue').textContent = '60%';
                    document.querySelector('#fanSpeed').closest('.control-item').querySelector('.control-status').textContent = '60%';
                    
                    // Turn on water pump if not already on
                    const waterPumpEvening = document.querySelector('.control-item label i.fa-shower').closest('.control-item').querySelector('.toggle-switch');
                ...
                case 'night':
                    ...
                    document.querySelector('#brightness').closest('.control-item').querySelector('.control-status').textContent = '20%';
                    
                    document.getElementById('fanSpeed').value = 30;
                    document.getElementById('fanSpeedValue').textContent = '30%';
                    document.querySelector('#fanSpeed').closest('.control-item').querySelector('.control-status').textContent = '30%';
                    
                    // Turn off water pump if not already off
                    const waterPumpNight = document.querySelector('.control-item label i.fa-shower').closest('.control-item').querySelector('.toggle-switch');
